...
PARAMETERS = 'InNode' + str(INPUT_NODE) + 'lr' + str(Learning_Rate) + '_Epoch'+str(Epoch) + '_lambda'+str(LAMBDA) + '_16examplaires'
PATH_PARAMETERS = './model/checkpoint_' + PARAMETERS + '.pth.tar'
...
# =================Data Processing========================
# Get all the data
X_np, Y_np, ranges, minValues = get_train_data()
np.savez('./model/Init_'+PARAMETERS+'.npz', Ranges=ranges, MinValues=minValues)
...
# X_np, Y_np = X_np[0:4, :], Y_np[0:4, :]

print("X Max: %f, Min: %f, Y Max: %f, Min: %f" % (X_np.max(), X_np.min(), Y_np.max(), Y_np.min()))
print("\nQuantity of train examples: {}\nX_train shape: {} Y_train shape: {}".format(X_train.shape[0], X_train.shape, Y_train.shape))
print("Quantity of validation examples: {}\nX_val shape: {} Y_val shape: {}".format(X_val.shape[0], X_val.shape, Y_val.shape))
    ...
    for step, (input, dose) in enumerate(train_loader):
        ...
        if count % 10000 == 0:
            ...
            if count % 50000 == 0:
                ...
                print("Epoch: {} Iteration: {}, Loss: {}, Loss_val : {}, Accuracy: {} %".format(epoch+1, count, round(loss.data.item(), 6), round(loss_val, 6), round(r2, 4)))
