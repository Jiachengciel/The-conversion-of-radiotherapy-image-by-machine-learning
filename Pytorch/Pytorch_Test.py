...
PATH_PARAMETERS_STATIC = './model_static/checkpoint_lr5e-05_Epoch80_lambda0.00011_5_6_7.pth.tar'
PATH_PARAMETERS_EXACT = './model_origin/checkpoint_lr5e-05_Epoch80_lambda0.0001.pth.tar'
...
def test(X, isStatic=False):
    ...
    if isStatic:
        ...
        PATH_PARAMETERS = PATH_PARAMETERS_EXACT
...
def get_results(isStatic=False):
    ...
    if isStatic:
        ...
        # TestExamples = [8, 10, 14]
        # TestExamples = [10, 11, 12, 14]
        TestExamples = [8, 9, 13, 15, 16, 17, 18]
        print("Test for exact: ", TestExamples)

    # For writing the results of Gamma Test
    # Todo
    # Change the path for multi test
    path_write = path_store + 'GAI_static'
    f = open(path_write, 'a')

    # Basic for Normalization of the test image
    if isStatic:
        _, _, ranges, minValues = get_train_data_static()
    else:
        _, _, ranges, minValues = get_train_data_origin()
    ...
    for i in TestExamples:
        ...
        X, Y_ = generate(Path_Img, isNormalize=False)
        # Normalization
        ...
        # Store the figure
        if isStatic:
            Path_Store = path_store + set_num + PATH_PARAMETERS_STATIC.split('/')[-1] + '.png'
        else:
            Path_Store = path_store + set_num + PATH_PARAMETERS_EXACT.split('/')[-1] + '.png'
...
if __name__ == '__main__':
    ...
    # # paint the figure
    # img_exact, pixel_size = get_image(path_test, isInput=False)
    # img_prediction = np.abs(np.reshape(Y_copy, img_exact.shape))
    #
    # gamma_map = Gamma_Map(img_prediction, img_exact, pixel_size, pixel_size)
    #
    # gamma_copy = gamma_map.copy()
