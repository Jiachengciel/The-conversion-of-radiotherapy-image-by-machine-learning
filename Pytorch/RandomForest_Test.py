...
path_store = './Result/Gamma_test_RF_static_meilleur_2/'

PATH_RF_EXACT = './model_RF/RandomForest_depth_26_estimator_19_features_11.pkl'
PATH_PARAMETERS_MY_STATIC = './model_RF/RandomForest_static_2_depth_26_estimator_19_features_11.pkl'
PATH_RF_KMEANS = './model_RF/RF3_kmeans/'
...
def test_kmeans(X, X_image, num_cluster=3):
    # Init
    Data0 = Data1 = Data2 = Data3 = Data4 = np.zeros([1, 18])
    ...
    for k in range(num_cluster):
        pos_data = cluster_new[k]
        Data_total[pos_data] = np.append(Data_total[pos_data], X[np.nonzero(assement == pos_data)[0]], axis=0)
        Data_total[pos_data] = np.delete(Data_total[pos_data], 0, axis=0)

    # =============== Prediction ========================
    Y = assement.astype(float)
    pos = assement.astype(str)

    for i in range(num_cluster):
        PATH_PARAMETERS = PATH_RF_KMEANS + 'RF3_centre_' + str(i) + '_depth_26_estimator_19_features_11.pkl'
        ...
        prediction = clf.predict(Data_total[i])

        Y[np.where(pos == str(i))] = prediction
...
def get_results(isStatic=False):
    ...
    for i in TestExamples:
        ...
        # X_image = get_image(Path_Img, isInput=True)[0]

        # Prediction with Model
        Y_pre = test(X, isStatic)
        Y_copy = Y_pre.copy()
        # Y_pre = test_kmeans(X, X_image, num_cluster=2)
        # Y_copy = Y_pre.copy()
